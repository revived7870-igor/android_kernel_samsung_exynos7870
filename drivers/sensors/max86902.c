	...
	u8 *buffer, int length)
{
	...
	if (err != 2) {
		pr_err("%s -read transfer error err = %d\n", __func__, err);
		err = -EIO;
	} else
		err = 0;
...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (onoff == HRM_LDO_ON) {
		...
		if (data->vdd_1p8_gpio) {
			gpio_set_value(data->vdd_1p8_gpio, HRM_LDO_OFF);
		} else {
		...
		if (data->led_3p3_gpio) {
			gpio_set_value(data->led_3p3_gpio, HRM_LDO_OFF);
		} else {
	...
	gpio_num = gpio_get_value(data->vdd_1p8_gpio);
	pr_info("%s - gpio_num 1p8 = %d", __func__, gpio_num);

	gpio_num = gpio_get_value(data->led_3p3_gpio);
	pr_info("%s - gpio_num = 3v3 %d", __func__, gpio_num);
	...
	struct device *dev)
{
	...
	data->vdd_1p8_gpio = of_get_named_gpio_flags(dNode,
		"max86900,vdd_1p8-gpio", 0, &flags);
	if (data->vdd_1p8_gpio < 0) {
		pr_err("%s - get vdd 1p8 error\n", __func__);
		return -ENODEV;
	}

	data->led_3p3_gpio = of_get_named_gpio_flags(dNode,
		"max86900,led_3p3-gpio", 0, &flags);
	if (data->led_3p3_gpio < 0) {
		pr_err("%s - get led 3p3 error\n", __func__);
		return -ENODEV;
	}
...
int max86900_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = sensors_create_symlink(&data->hrmled_input_dev->dev.kobj,
		data->hrmled_input_dev->name);
	if (err < 0) {
		pr_err("%s - create_symlink error\n", __func__);
		goto err_hrm_sensors_create_symlink;
	}
