 ...
 */
#define OVERRIDE_CRED(sdcardfs_sbi, saved_cred)	do { \
...
#define OVERRIDE_CRED_PTR(sdcardfs_sbi, saved_cred) do { \
	saved_cred = override_fsids(sdcardfs_sbi->options.fs_low_uid, \
								sdcardfs_sbi->options.fs_low_gid); \
	if (!saved_cred) { \
...
#define OVERRIDE_ROOT_CRED(saved_cred) do { \
...
out_nospc:
	printk_ratelimited("statfs.f_bavail : %llu blocks / "
